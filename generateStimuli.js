function generateRandomStimuli(relevantType, frequentType, secondFrequentType){
	arrays = returnArrays()
	indoor = arrays[0]
	outdoor = arrays[1]
	male = arrays[2]
	...
	finalStimuliArray = []
	for (i=0; i<300; i++){
		finalStimuliArray.push({Scene: shuffledSceneArray[i].path, sceneType: shuffledSceneArray[i].type, Face: shuffledFaceArray[i].path, faceType: shuffledFaceArray[i].type, randomVariable: Math.floor(Math.random()*2)})
	}
	console.log(shuffledMemArray)
	console.log(finalStimuliArray)
...
function generateNewMemory(shuffledSceneArray, shuffledFaceArray, relevantType, frequentType, secondFrequentType, male, female, outdoor, indoor){
	console.log(relevantType, frequentType, secondFrequentType)
	...
	console.log(relevantFrequentWithInfrequent.length,
		relevantFrequentWithFrequent.length,
		relevantInfrequentWithFrequent.length,
		irrelevantInfrequentWithFrequent.length,
		irrelevantFrequentWithFrequent.length,
		irrelevantFrequentWithInfrequent.length)
	relevantFrequentLure = []
	irrelevantFrequentLure = []
	relevantInfrequentLure = []
	irrelevantInfrequentLure = [] 
	console.log(male.length, female.length, outdoor.length, indoor.length)
	...
	totalMemTrials = []
	for (i = 0; i < 25; i++){
		totalMemTrials.push(convertSeenImage(relevantFrequentWithInfrequent[i], 'relevant', 'frequent', 'infrequent', shuffledSceneArray, shuffledFaceArray))
		totalMemTrials.push(convertSeenImage(relevantFrequentWithFrequent[i], 'relevant', 'frequent', 'frequent', shuffledSceneArray, shuffledFaceArray))
		totalMemTrials.push(convertSeenImage(relevantInfrequentWithFrequent[i], 'relevant', 'infrequent', 'frequent', shuffledSceneArray, shuffledFaceArray))
		totalMemTrials.push(convertSeenImage(irrelevantInfrequentWithFrequent[i], 'irrelevant', 'infrequent', 'frequent', shuffledSceneArray, shuffledFaceArray))
		totalMemTrials.push(convertSeenImage(irrelevantFrequentWithFrequent[i], 'irrelevant', 'frequent', 'frequent', shuffledSceneArray, shuffledFaceArray))
		totalMemTrials.push(convertSeenImage(irrelevantFrequentWithInfrequent[i], 'irrelevant', 'frequent', 'infrequent', shuffledSceneArray, shuffledFaceArray))
		totalMemTrials.push(convertNewImage(irrelevantInfrequentLure[i], 'irrelevant', 'infrequent'))
		totalMemTrials.push(convertNewImage(relevantInfrequentLure[i], 'relevant', 'infrequent'))
	}
	for (q = 0; q < 50; q++){
		totalMemTrials.push(convertNewImage(relevantFrequentLure[q], 'relevant', 'frequent'))
		totalMemTrials.push(convertNewImage(irrelevantFrequentLure[q], 'irrelevant', 'frequent'))
	}
	shuffledMemTrials = shuffle(totalMemTrials)
	return shuffledMemTrials
}
...
function generatePracticeStimuli(){
	...
	faces = [
		...
		{type: 'male', path: 'images/male/CFD-BM-026-017-A.jpg'},		
	...
	finalStimuliArray = []
	for (i = 0; i < shuffledFaceArray.length; i++){
		finalStimuliArray.push({Scene: shuffledSceneArray[i].path, sceneType: shuffledSceneArray[i].type, Face: shuffledFaceArray[i].path, faceType: shuffledFaceArray[i].type, randomVariable: Math.floor(Math.random()*2)})
	}
	return finalStimuliArray
